...
describe("LoginScreen component", () => {
  ...
  describe('when form is properly filled', () => {
    it('should show incorrect data error when backend returns 401 code when' +
      ' clicking to login', async () => {
      loginFn.mockReturnValue(Promise.reject({
        response: {
          status: 401
        }
      }));
      ...
      await waitFor(() => {
        fireEvent.changeText(getByTestId('login-email-or-username'), 'fulanodetal');
    ...
    it('should show incorrect data error when backend returns 400 code when' +
      ' clicking to login', async () => {
      loginFn.mockReturnValue(Promise.reject({
        response: {
          status: 400
